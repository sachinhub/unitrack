...
export class ShipwayProvider implements TrackingProvider {
  name = 'Shipway';
  ...
  async track(trackingNumber: string): Promise<TrackingResult> {
    ...
    const firstBatchPromises = firstBatch.map(customer => 
      this.tryCustomerSubdomain(customer, trackingNumber).catch(() => null)
    );
  ...
  private async tryCustomerSubdomain(customer: string, trackingNumber: string): Promise<TrackingResult> {
    try {
  ...
  private parseTrackingResponse(htmlData: string, trackingNumber: string): TrackingResult {
    try {
